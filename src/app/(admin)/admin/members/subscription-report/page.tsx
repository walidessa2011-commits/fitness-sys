...
export default function SubscriptionReportPage() {
    ...
    return (
        <div className="flex flex-col gap-4 animate-in fade-in duration-500 max-w-full mx-auto pb-4" dir="rtl">
            <div className="flex flex-col gap-4 print:hidden">

                {/* Header Section */}
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 shadow-sm border border-gray-300 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        <div className="w-12 h-12 bg-gradient-to-br from-indigo-600 to-blue-700 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-blue-500/20">
                            <BarChart3 className="w-6 h-6" />
                        </div>
                        <div>
                            <h1 className="text-xl font-black text-slate-900 dark:text-white leading-tight">تقرير وتحليل الاشتراكات</h1>
                            <p className="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mt-0.5 flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 relative z-10">
                        ...
                        <div className="flex gap-2 no-print">
                            <button
                                onClick={exportToPDF}
                                className="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded-xl font-black text-xs shadow-lg shadow-rose-500/10 transition-all flex items-center gap-2 active:scale-95 group"
                            >
                            ...
                            <button
                                ...
                                className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl font-black text-xs shadow-lg shadow-emerald-500/10 transition-all flex items-center gap-2 active:scale-95 group"
                            >
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
                    <StatCard label="إجمالي الاشتراكات" value={stats.total} icon={<TrendingUp className="w-4 h-4" />} color="indigo" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-4">
                        <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 shadow-sm border border-gray-300 dark:border-slate-800">
                            ...
                            <div className="space-y-1.5">
                                {activityStats.length > 0 ? activityStats.map((item, idx) => (
                                    <div key={item.id} className="group flex flex-col gap-1.5 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all border border-transparent hover:border-slate-300 dark:hover:border-slate-700">
                        ...
                        {/* Paused Quick List */}
                        <div className="bg-rose-50/50 dark:bg-rose-900/10 rounded-3xl p-4 border border-rose-100/50 dark:border-rose-900/20">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4">
                        <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 shadow-sm border border-gray-300 dark:border-slate-800 overflow-hidden">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-right">
                                    <thead className="bg-indigo-600 dark:bg-indigo-700 text-white shadow-sm text-[10px] font-black uppercase tracking-widest h-10">
                                        <tr>
                                            <th className="px-4 py-2 text-right first:rounded-tr-2xl border-l border-white/5 last:border-l-0 w-[30%]">المشترك</th>
                                            <th className="px-4 py-2 text-center border-l border-white/5 last:border-l-0">رقم العضوية</th>
                                        ...
                                        {loading ? (
                                            ...
                                            filteredResults.map((item, idx) => (
                                                <tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all duration-300">
                                                    <td className="px-4 py-3 border-l border-gray-100/20 last:border-l-0">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="text-xs font-black text-gray-700 dark:text-slate-200">{item.memberName}</span>
                                                                <span className="text-[9px] font-bold text-gray-400">{item.phone || 'بدون رقم'}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-center border-l border-gray-100/20 last:border-l-0">
                                                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded text-[10px] font-black">
                                                            #{item.membershipNumber}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3 text-center border-l border-gray-100/20 last:border-l-0">
                                                        <div className="flex flex-col items-center">
                                                            <span className="text-xs font-black text-gray-700 dark:text-slate-200">{item.activityName}</span>
                                                            <span className="text-[9px] font-bold text-indigo-500">{item.activityType}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-center border-l border-gray-100/20 last:border-l-0">
                                                        <div className="flex items-center justify-center gap-2">
                                                            <User className="w-3 h-3 text-gray-400" />
                                                            <span className="text-xs font-bold text-gray-600 dark:text-slate-400">{item.coachName}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-center border-l border-gray-100/20 last:border-l-0">
                                                        <div className="flex flex-col items-center">
                                                            <span className="text-[10px] font-black text-gray-700 dark:text-slate-200" dir="ltr">{item.endDate}</span>
                                                            <span className={`text-[8px] font-bold ${new Date(item.endDate) < new Date() ? 'text-rose-500' : 'text-emerald-500'}`}>
                                                                {new Date(item.endDate) < new Date() ? 'منتهي' : 'متبقي'}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3 text-center border-l border-gray-100/20 last:border-l-0">
                                                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${item.status === 'نشط' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                                                            item.status === 'موقوف' || item.status === 'منتهي' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' :
                                            ...
                                            <tr>
                                                <td colSpan={6} className="py-32 text-center border-l border-gray-100/20 last:border-l-0">
...
function StatCard({ label, value, icon, color }: any) {
    ...
    return (
        <motion.div
            whileHover={{ y: -5 }}
            className={`p-3 rounded-2xl border shadow-sm transition-all flex flex-col gap-2 relative overflow-hidden group ${colors[color]}`}
        >
            <div className="flex items-center justify-between">
                <div className="w-8 h-8 rounded-xl bg-white dark:bg-slate-900/50 flex items-center justify-center shadow-inner group-hover:rotate-12 transition-transform">
                    {icon}
                </div>
                <MoreHorizontal className="w-4 h-4 opacity-20" />
            </div>
            <div>
                <span className="text-[9px] font-black uppercase tracking-widest opacity-60 decoration-slate-300">{label}</span>
                <div className="text-xl font-black mt-0.5">{value.toLocaleString()}</div>
            </div>
