...
export default function RevenuesPage() {
    ...
    }, [searchQuery, typeFilter, clubFilter, viewMode, selectedDate, startDate, endDate, revenues]);
            ...
            <div className="print:hidden flex flex-col gap-2.5 w-full">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-2 px-3 shadow-sm border border-gray-300 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-[11px] font-black transition-all ${viewMode === 'today' ? 'bg-white dark:bg-slate-700 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                إيرادات اليوم
                            </button>
                            <button
                                onClick={() => setViewMode('specific')}
                                className={`px-4 py-1.5 rounded-lg text-[11px] font-black transition-all ${viewMode === 'specific' ? 'bg-white dark:bg-slate-700 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                يوم محدد
                            </button>
                            <button
                                onClick={() => setViewMode('range')}
                                className={`px-4 py-1.5 rounded-lg text-[11px] font-black transition-all ${viewMode === 'range' ? 'bg-white dark:bg-slate-700 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                فترة محددة
                            </button>
                            <button
                                onClick={() => setViewMode('all')}
                                className={`px-4 py-1.5 rounded-lg text-[11px] font-black transition-all ${viewMode === 'all' ? 'bg-white dark:bg-slate-700 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                الكل
                            </button>
                        </div>
                        ...
                        {viewMode === 'range' && (
                            <div className="flex items-center gap-3 animate-in zoom-in-95 duration-200">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="w-36">
                                        <DatePicker value={startDate} onChange={setStartDate} className="h-8 text-[11px] font-black border-none ring-1 ring-gray-200" />
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-black text-gray-400 uppercase">إلى:</span>
                                    <div className="w-36">
                                        <DatePicker value={endDate} onChange={setEndDate} className="h-8 text-[11px] font-black border-none ring-1 ring-gray-200" />
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={exportToPDF}
                                className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-lg text-[10px] font-black text-gray-400 hover:text-emerald-600 transition-all font-tajawal"
                            >
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-2 px-3 shadow-sm border border-gray-300 dark:border-slate-800 flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2 border-r pr-3 border-gray-300 dark:border-slate-800">
                        ...
                        <button className="w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-lg text-gray-400 hover:text-emerald-600 transition-all invisible">
                            ...
                            {filteredRevenues.length > 0 && (
                                <tfoot className="bg-slate-50 dark:bg-slate-800/80 border-t border-gray-300 dark:border-slate-800">
                                    <tr className="divide-x divide-x-reverse divide-gray-300 dark:divide-slate-700">
                                        <td colSpan={5} className="px-5 py-4 text-[13px] font-black text-slate-800 dark:text-slate-200 text-left">إجمالي الإيرادات المفلترة :</td>
                                        <td className="px-5 py-4 text-center">
                                            <div className="text-[14px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 py-1 rounded-xl border border-emerald-100 dark:border-emerald-800" dir="ltr">
                ...
                <DeleteModal
                    ...
                    title={deleteMode === 'entry' ? "حذف عملية إيراد" : "حذف تصنيف إيرادات"}
            ...
            <div className="hidden print:block bg-white text-black rtl w-full" dir="rtl">
                <div className="flex justify-between items-center border-b-2 border-slate-900 pb-2 mb-4">
                    <div className="flex-1 text-right">
                        ...
                        <p className="text-[11px] font-bold text-slate-500 mb-0.5">تقرير الإيرادات المالي</p>
                        ...
                        {viewMode === 'range' ? (
                            ...
                            <p className="text-[10px] font-bold mt-1">كافة الإيرادات</p>
                        )}
                    </div>
                ...
                <div className="flex justify-end mt-6">
                    <div className="w-72 space-y-2.5">
                        <div className="flex justify-between items-center p-2 border-b border-slate-200">
                            ...
                            <span className="text-[11px] font-black">{filteredRevenues.filter(r => r.paymentMethod === 'نقدي').reduce((acc, r) => acc + Number(r.amount), 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</span>
                        </div>
                        <div className="flex justify-between items-center p-2 border-b border-slate-200">
                            <span className="text-[10px] font-black text-slate-500">إجمالي الشبكة (Network)</span>
                            <span className="text-[11px] font-black">{filteredRevenues.filter(r => r.paymentMethod === 'شبكة').reduce((acc, r) => acc + Number(r.amount), 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</span>
                        </div>
                        <div className="flex justify-between items-center p-2 border-b border-slate-200">
                            <span className="text-[10px] font-black text-slate-500">إجمالي التحويل (Transfer)</span>
                            <span className="text-[11px] font-black">{filteredRevenues.filter(r => r.paymentMethod === 'تحويل بنكي').reduce((acc, r) => acc + Number(r.amount), 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</span>
                        </div>
