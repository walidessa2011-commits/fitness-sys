...
export default function ExpensesPage() {
    ...
    }, [searchQuery, typeFilter, clubFilter, expenses]);
    ...
    return (
        <div className="flex flex-col gap-2.5 animate-in fade-in duration-500 max-w-full mx-auto pb-10" dir="rtl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <MiniStatCard label="مصاريف اليوم" value={stats.today} icon={<Calendar className="w-4 h-4 text-rose-500" />} color="rose" />
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-2 px-3 shadow-sm border border-gray-300 dark:border-slate-800 flex flex-wrap items-center gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2 border-r pr-3 border-gray-300 dark:border-slate-800">
                    ...
                    <button className="w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-lg text-gray-400 hover:text-rose-600 transition-all">
            ...
            <DeleteModal
                ...
                title={deleteMode === 'entry' ? "حذف المصروف" : "حذف تصنيف المصروفات"}
                message={deleteMode === 'entry'
                    ? "هل أنت متأكد من رغبتك في حذف هذا المصروف؟ لا يمكن التراجع عن هذه العملية."
                    : "هل أنت متأكد من حذف هذا التصنيف؟ قد يؤثر ذلك على تجميع البيانات المرتبطة به."}
                confirmText={deleteMode === 'entry' ? "نعم، حذف المصروف" : "نعم، حذف التصنيف"}
